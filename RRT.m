classdef RRT
    ...
    methods
        ...
        function obj = RRT(x_i,y_i,teta_i, x_f, y_f, teta_f,x_min,x_max,y_min,y_max,obstacles,robot)
        ...
        % ostacoli
        function [x_r,y_r,q_r] = sampleFree(obj)
            counter = 0;
            [x,y,q] = obj.robot.getRandomConfig(obj.boundaries(1:2),obj.boundaries(3:4));
            while(counter<1000)
                flag = false;
                for i=1:size(obj.obstacles(1))
                    if(~isempty(obj.obstacles(i).intersect(x,y)))
                        flag = true;
                    end
                end
                if(flag==false)
        ...
        function nearestNode = getNearestConf(obj,X_random)
            ...
            for i=1:size(obj.nodes(1))
                cart_dist = sqrt(sqr(x_r-obj.nodes(i,1))+ sqr(y_r-obj.nodes(i,2)));
        ...
        function newNode = steer(obj,nearestNode)
            configurations = obj.robot.directKynematics(nearestNode);
