    ...
    methods
        ...
        function qtarget = chooseTarget(obj)
            ...
            while(counter < 1000)
                ...
                if(norm(obj.init_node(1,1:2)-W(1:2))+norm(W(1:2)-obj.final_node(1,1:2))>obj.upper_bound||obj.checkCollision(W))
                    counter = counter + 1;
                else
                    qtarget = W;
                    return;
        ...
        function newNode = generateExtension(obj,qact,qgoal) %qui suppongo qact e qgoal siano colonne
            delta = 0.8;
            intersections = obj.solveIntersection(qact(1),qgoal(1),qact(2),qgoal(2),qact(1),qact(2),delta);
            ...
            %             configurations = obj.robot.directKinematics(qact);
            %             minDist = Inf(1);
            %             for c = configurations
            %                 dist = norm(c(1:2)-qgoal(1:2));
            %                 if dist < minDist
            %                     minDist = dist;
            %                     newNode = c;
            %                 end
       ...
       % end
        ...
        function obj = addNewNode(obj,qnew,newcost,qnext) %suppongo new e next siano righe
            if(size(qnew,1)>1)
                qnew = qnew';
            end
            if(size(qnext,1)>1)
        ...
        function X = extendToTarget(obj,qtarget)
            ...
            while(~isempty(select))
                ...
                q_new = q_new';
                path = shortestpath(obj.graph,1,obj.findId(qtree));
                cost_start_tree = 0;
                for i=2:size(path,2)
                    % if(obj.nodes(i,3)==0)
                    cost_start_tree = cost_start_tree + obj.robot.v;
                    % else
                    %     cost_start_tree = cost_start_tree + pi*(obj.robot.v/sqrt(2))/2;
                    % end
                end
                tempcost = cost_start_tree+norm(qtree(1,1:2)-q_new(1,1:2));
                if(tempcost+norm(q_new(1,1:2)-obj.final_node(1,1:2))<obj.upper_bound)
                    qnew = q_new;
                    cost_qnew = tempcost;
                    qnear = qtree;
            ...
            while(nor>distance_treshold)
                ...
                if(q_target ~= -1)
                    ...
                    if(new~=-1)
                        ...
                        q_new_cost = new(1,4);
                        q_near = new(1,5:7);
                        if obj.obstacleFree(q_new,q_near)
                ...
                if(time>max_time)
                    ...
                    for i=2:size(path,2)
                        ...
                        cost = cost + norm(qpred(1:2)-qactual(1:2));
                    end
